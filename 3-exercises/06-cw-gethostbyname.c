/**
 * Utilizza la chiamata alla libreria di sistema gethostbyname()
 * per risolvere il nome di dominio e ottenere l'indirizzo IP
 * del target host per la connessione
 */
...
#include <stdio.h>
#include <string.h>
#include <netdb.h>      //Include needed to use gethostbyname
...
int main()
{
    ...
    //START OF MODIFICATION: resolve the address starting from the name
    printf("Coded address: %d\n",*targetip);
    struct hostent* he;
    he = gethostbyname("google.com");
    *targetip = *(unsigned int *)he->h_addr_list[0];
