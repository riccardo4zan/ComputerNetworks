...
int main()
{
    unsigned char addresses[4][4] = {{192, 168, 1, 1}, {88,88,88,88}, {79,13,122,14}, {1,2,3,4}};
    ...
    while( 1 ){
        ...
        // if the address requested is in our list, filter contents that are not txt or html files
        // returning a 403 Forbidden code to the client
        int filtered = 0;
        for(int i=0; i<4; i++)
            for(int j=0; j<4; j++) 
                if(*addresses[i] == (unsigned char) remote.sin_addr.s_addr) 
                    filtered = 1;
        ...
        for(ext=1; ext < strlen(path); ext++) if(path[ext-1]=='.') break;
        for(; ext < strlen(path); ext++) if(path[ext-1]=='/') break;
        for(; ext < strlen(path); ext++) if(path[ext-1]=='.') break;

        if(!filtered || !strcmp(path+ext, "txt") || !strcmp(path+ext, "html")) { 
            ...
            sprintf(response2, "HTTP/1.1 403 Forbidden\r\n\r\n");
            write(s2, response2, 26);
        }
